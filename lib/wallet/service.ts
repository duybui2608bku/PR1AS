...
export class WalletService {
  ...
  async getPlatformSettings(): Promise<PlatformSettings> {
    ...
    if (error) {
      throw new WalletError('Failed to fetch platform settings', 'SETTINGS_ERROR', 500);
    }
  ...
  async updatePlatformSettings(
    ...
    if (error) {
      throw new WalletError('Failed to update platform settings', 'SETTINGS_UPDATE_ERROR', 500);
    }
  }
  ...
  async calculateFees(amount: number): Promise<FeeCalculation> {
    ...
    if (settings.payment_fees_enabled) {
      platform_fee = Math.round((amount * settings.platform_fee_percentage / 100) * 100) / 100;
      insurance_fee = Math.round((amount * settings.insurance_fund_percentage / 100) * 100) / 100;
    }
  ...
  async getOrCreateWallet(userId: string): Promise<Wallet> {
    ...
    if (error && error.code === 'PGRST116') {
      ...
      if (createError) {
        throw new WalletError('Failed to create wallet', 'WALLET_CREATE_ERROR', 500);
      }

      wallet = newWallet;
    } else if (error) {
      throw new WalletError('Failed to fetch wallet', 'WALLET_FETCH_ERROR', 500);
    }
  ...
  async getWallet(userId: string): Promise<Wallet> {
    ...
    if (error || !data) {
      throw new WalletError('Wallet not found', WalletErrorCodes.WALLET_NOT_FOUND, 404);
    }
  ...
  async checkBalance(userId: string, amount: number): Promise<boolean> {
    ...
    if (wallet.status !== 'active') {
      throw new WalletError('Wallet is not active', WalletErrorCodes.WALLET_FROZEN, 403);
    }
  ...
  private async updateWalletBalance(
    ...
    if (balanceChange > 0) {
      ...
      updateData.total_spent_usd = wallet.total_spent_usd + Math.abs(balanceChange);
    }
    ...
    if (error) {
      throw new WalletError('Failed to update wallet balance', 'WALLET_UPDATE_ERROR', 500);
    }
  ...
  async createTransaction(params: {
    ...
    if (error) {
      throw new WalletError('Failed to create transaction', WalletErrorCodes.TRANSACTION_FAILED, 500);
    }
  ...
  async updateTransactionStatus(
    ...
    if (error) {
      throw new WalletError('Failed to update transaction', 'TRANSACTION_UPDATE_ERROR', 500);
    }
  ...
  async getTransactions(filters: TransactionFilters): Promise<{ transactions: Transaction[], total: number }> {
    ...
    if (error) {
      throw new WalletError('Failed to fetch transactions', 'TRANSACTION_FETCH_ERROR', 500);
    }
   ...
   */
  async processPayment(payment: PaymentRequest): Promise<{ escrow: EscrowHold, transaction: Transaction }> {
    const { employer_id, worker_id, job_id, amount_usd, description, cooling_period_days } = payment;

    // Validate amount
    if (amount_usd <= 0) {
      throw new WalletError('Invalid payment amount', WalletErrorCodes.INVALID_AMOUNT, 400);
    }
    ...
    const settings = await this.getPlatformSettings();
    const coolingPeriod = cooling_period_days || settings.escrow_cooling_period_days;
    ...
    try {
      ...
      // 4. Update transaction with escrow ID
      await this.supabase
        .from('transactions')
      ...
      return {
        ...
        transaction: { ...transaction, escrow_id: escrow.id },
      };
    } catch (error) {
   ...
   */
  async releaseEscrow(escrowId: string, adminId?: string): Promise<Transaction> {
    // Get escrow
    const { data: escrow, error: escrowError } = await this.supabase
      .from('escrow_holds')
      .select('*')
    ...
    if (escrowError || !escrow) {
      throw new WalletError('Escrow not found', WalletErrorCodes.ESCROW_NOT_FOUND, 404);
    }
    ...
    if (!adminId && new Date(escrowHold.hold_until) > new Date()) {
      throw new WalletError('Cooling period not yet complete', 'COOLING_PERIOD_ACTIVE', 400);
    }

    try {
      // 1. Add money to worker wallet
      await this.updateWalletBalance(escrowHold.worker_id, escrowHold.worker_amount_usd);
      ...
      throw new WalletError('Failed to release escrow', 'ESCROW_RELEASE_ERROR', 500);
    }
  }
   ...
   */
  async fileComplaint(complaint: ComplaintRequest, userId: string): Promise<EscrowHold> {
   ...
   */
  async resolveComplaint(resolution: ComplaintResolution, adminId: string): Promise<EscrowHold> {
    const { escrow_id, action, worker_amount, employer_refund, resolution_notes } = resolution;
    ...
    try {
      if (action === 'release_to_worker') {
        ...
        await this.updateWalletBalance(escrowHold.employer_id, escrowHold.total_amount_usd);
        ...
        if (!worker_amount || !employer_refund) {
          throw new WalletError('Worker amount and employer refund required for partial refund', 'INVALID_RESOLUTION', 400);
        }
      ...
      throw new WalletError('Failed to resolve complaint', 'RESOLUTION_ERROR', 500);
    }
  }
  ...
  async getEscrowsReadyForRelease(): Promise<EscrowHold[]> {
    ...
    if (error) {
      throw new WalletError('Failed to fetch escrows for release', 'ESCROW_FETCH_ERROR', 500);
    }
   ...
   */
  async getEscrows(filters: EscrowFilters): Promise<{ escrows: EscrowHold[], total: number }> {
    let query = this.supabase
      .from('escrow_holds')
      .select('*', { count: 'exact' })
      .order('created_at', { ascending: false });
    ...
    if (error) {
      throw new WalletError('Failed to fetch escrows', 'ESCROW_FETCH_ERROR', 500);
    }

    return {
      escrows: data as EscrowHold[],
      total: count || 0,
    };
  }
  ...
  async getAdminStats(): Promise<AdminWalletStats> {
    ...
    const total_balance = balanceData?.reduce((sum, w) => sum + Number(w.balance_usd), 0) || 0;
    const total_pending = balanceData?.reduce((sum, w) => sum + Number(w.pending_usd), 0) || 0;
    ...
    const platform_revenue = feeData?.reduce((sum, t) => sum + Number(t.amount_usd), 0) || 0;
    ...
    const insurance_fund = insuranceData?.reduce((sum, t) => sum + Number(t.amount_usd), 0) || 0;
